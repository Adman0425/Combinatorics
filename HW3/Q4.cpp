...
long long edmonds_karp(long long graph[V][V], int s, int t, int n, vector<pair<int, int>>& min_cut_edges)
{
    ...
    // Find the edges crossing the minimum s-t cut
    for (int u = 0; u < n; u++)
    {
        for (int v = 0; v < n; v++)
        {
...
// Prints the minimum s-t cut edges
void printMinCut(const vector<pair<int, int>>& min_cut_edges)
{
    for (const auto& edge : min_cut_edges)
    {
...
int main()
{
    ...
    long long max_flow = edmonds_karp(graph, s, t, n, min_cut_edges);

    // Print the maximum flow
    cout << max_flow << endl;

    // Print the minimum s-t cut edges
    printMinCut(min_cut_edges);
